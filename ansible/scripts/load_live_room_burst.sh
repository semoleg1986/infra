...
K6_LIVE_OFFER_ID="${K6_LIVE_OFFER_ID:-${K6_PAYMENT_OFFER_ID:-}}"
K6_LIVE_COURSE_ID="${K6_LIVE_COURSE_ID:-}"
K6_LIVE_LESSON_ID="${K6_LIVE_LESSON_ID:-}"
K6_VUS="${K6_VUS:-10}"
...
if [[ -z "${K6_LIVE_OFFER_ID}" ]]; then
  log "ERROR K6_LIVE_OFFER_ID or K6_PAYMENT_OFFER_ID is required"
  exit 1
fi

if [[ -z "${K6_LIVE_LESSON_ID}" ]]; then
  log "ERROR K6_LIVE_LESSON_ID is required"
  exit 1
fi
...
log "K6_LIVE_OFFER_ID=${K6_LIVE_OFFER_ID}"
log "K6_LIVE_COURSE_ID=${K6_LIVE_COURSE_ID:-resolved-from-offer}"
log "K6_LIVE_LESSON_ID=${K6_LIVE_LESSON_ID}"
log "K6_VUS=${K6_VUS}"
...
exec docker run --rm \
  ...
  -e K6_LIVE_OFFER_ID="${K6_LIVE_OFFER_ID}" \
  -e K6_LIVE_COURSE_ID="${K6_LIVE_COURSE_ID}" \
  -e K6_LIVE_LESSON_ID="${K6_LIVE_LESSON_ID}" \
  -e K6_VUS="${K6_VUS}" \
